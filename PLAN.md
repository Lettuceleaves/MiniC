...
目标：扩展 `MiniType`，表达函数签名类型以及指向函数签名的指针类型，为后续声明、赋值和调用做准备。

验收：类型模型可区分数据指针和函数指针；函数签名包含返回类型和参数类型列表；测试覆盖类型构造、相等性和字符串表示。
...
目标：支持函数指针变量、参数和返回类型的最小声明语法。

验收：parser 能解析代表性函数指针声明；非法声明产出 diagnostics；不要求本任务完成调用代码生成。

验证：`./gradlew test`

### A132：添加函数地址和函数指针赋值语义

依赖：A131。

目标：支持将函数名作为函数地址使用，并将兼容签名的函数地址赋给函数指针变量。

验收：函数指针赋值执行签名兼容检查；不兼容签名产出 diagnostics；测试覆盖合法和非法赋值。
...
目标：支持 `fp(args...)` 形式的间接调用，并生成目标平台间接调用汇编。

验收：函数指针调用遵守现有调用约定；样例程序可通过函数指针调用普通函数并返回正确结果；测试覆盖 IR 和 codegen。
...
目标：支持函数指针作为函数参数传递，并评估是否支持函数指针返回值；若暂不支持返回值，需形成明确 diagnostics 和后续任务。

验收：函数指针参数可端到端工作；函数指针返回值的支持或限制有测试覆盖；README 记录样例预期。
...
目标：扩展 `MiniType` 和布局计算，让 `bool`、有符号 `char`、`int`、`long`、`float`、`double` 具备稳定的大小、对齐、是否有符号、是否浮点等属性，并让 `NULL` 拥有专门的空指针常量类型。

验收：结构体布局、数组元素大小、局部变量槽位大小使用类型尺寸；已有 `int` 行为保持不变；测试覆盖各基础类型尺寸、对齐和 `NULL` 类型。
...
目标：支持 `bool`、`char`、`int`、`long`、`float`、`double` 的声明语法，以及布尔、字符、整数、长整数、单精度/双精度浮点和 `NULL` 字面量形式。

验收：lexer/parser 能识别新增关键字和字面量；语义分析能记录变量、参数和返回类型；非法字面量产出 diagnostics；`NULL` 只能作为空指针常量参与指针相关上下文。
...
目标：实现赋值、返回、函数实参、二元运算、比较运算中的标量类型兼容和转换规则，包括整数提升、`long` 提升、浮点提升、`float` 到 `double`、以及 `NULL` 到任意指针类型的转换。

验收：合法转换通过；有损或非法转换按 `SPEC.md` 规则处理；测试覆盖 `bool`、`char`、`int`、`long`、`float`、`double`、`NULL`、指针和结构体边界。

验证：`./gradlew test`

### A143：扩展 IR 和代码生成的标量宽度

依赖：A142。

目标：让 IR 和 Windows x64 codegen 支持 `bool`、有符号 `char`、`int`、`long` 的整数宽度、返回寄存器宽度、栈槽大小、符号扩展和 load/store 指令宽度，并支持 `NULL` 作为零地址常量。

验收：`bool/char/int/long` 可生成正确汇编；结构体字段偏移随字段类型尺寸变化；`NULL` 可赋给和传递给指针；测试覆盖关键汇编片段。
...
目标：补充 `float`、`double` 的 IR、常量表示、SSE 寄存器约定、算术、比较、转换、返回值和函数调用代码生成。

验收：浮点样例可端到端编译运行；`float` 和 `double` 的变量、参数、返回值、算术、比较、结构体字段和数组元素有测试覆盖。

验证：`./gradlew test`

## Phase 15：Debugger 模型

### A150：添加 Debugger 命令和快照模型
