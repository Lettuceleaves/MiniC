...
`0.2.0` 验收状态：`B000-B060` 已完成，阶段收口任务为 `B061`。下一阶段候选方向为 JavaFX UI、运行时 debugger、真实进程调试和反向 rr debug。
...
- `samples/**` 作为 `0.1.0` 端到端语言行为基线，按退出码、stdout 或代表性 diagnostic 验收。
- `MiniCompilerTest` 作为 pipeline 基线，保留合法编译、词法/语义 diagnostic 短路、工具链产物和 compile-run 反馈。
- 后续 B1-B5 结构化任务优先测试阶段数据、步进状态、调度和 UI API 契约；除非暴露真实回归，不再为既有语法能力补大量单语法细节测试。
