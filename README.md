...
MiniC 是一个用于学习编译原理的 Java 版 C 语言子集编译器。当前已经完成从源码到真实可执行文件的基础编译闭环，并完成 `0.2.0` 结构化观测阶段，具备编译层可步进改造和面向 UI 的统一调度接口。

当前版本：`0.2.0`。稳定基线版本 `0.1.0` 已完成第一阶段编译器闭环，阶段总结见 [version/0.1.0.md](version/0.1.0.md)。`0.2.0` 阶段总结见 [version/0.2.0.md](version/0.2.0.md)，执行记录见 [PLAN.md](PLAN.md) 的 B 系列任务。
...
`0.2.0` 阶段已完成。当前已经具备从 MiniC 源码到 Windows x64 可执行文件的基础编译运行闭环，并完成函数、控制流、外部函数、字符串、指针、数组、结构体、函数指针参数、基础类型系统、浮点代码生成。函数指针返回值当前明确诊断为暂不支持。类型系统阶段确认纳入 `bool`、有符号 `char`、`int`、`long`、`float`、`double` 和 `NULL`，其中 MiniC 采用 `long` 为 8 字节。

`0.2.0` 阶段没有继续扩展 C 语言能力，已完成以下结构化改造：
...
B040 已完成：已添加编译观测会话，支持从源码创建，持有阶段顺序、当前阶段、当前阶段 stepper、全局步骤计数和各阶段结果缓存。
B041 已完成：调度层已实现全局 `next`，按阶段顺序推进当前阶段最小步骤，并在阶段完成后的下一次调用进入下一个阶段，状态数据、阶段数据和全局数据同步更新。
...
B061 已完成：`0.2.0` 阶段验收通过，结构化观测和 UI API 达到可用状态，下一阶段候选为 JavaFX UI、运行时 debugger、真实进程调试和反向 rr debug。

下一阶段候选：JavaFX UI、运行时 debugger、真实进程调试、反向 rr debug。
