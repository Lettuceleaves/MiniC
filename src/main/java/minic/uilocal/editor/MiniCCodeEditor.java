...
public final class MiniCCodeEditor extends StackPane {
    ...
    private final MiniCEditorFormatter formatter = new MiniCEditorFormatter(input);
    ...
    public MiniCCodeEditor() {
        ...
        input.addEventFilter(KeyEvent.KEY_TYPED, formatter::handleTypedText);
    ...
    private void handleCompletionKeys(KeyEvent event) {
        ...
        if (event.getCode() == KeyCode.BACK_SPACE) {
            formatter.deleteBackward();
            event.consume();
            return;
        }
        if (event.getCode() == KeyCode.ENTER) {
            formatter.insertNewlineWithIndent();
            event.consume();
            return;
        }
    }
    ...
    private void updateCompletion(boolean force) {
        ...
        List<String> suggestions = MiniCCompletionSuggester.suggestions(
                prefix.text(),
                input.getText(),
                latestAnalysis == null ? List.of() : latestAnalysis.tokens()
        );
